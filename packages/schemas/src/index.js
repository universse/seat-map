...
SeatMap.read = function (pbf, end) {
  return pbf.readFields(
    SeatMap._readField,
    { svgProps: null, seatProps: null, groups: [] },
    end
  )
}
SeatMap._readField = function (tag, obj, pbf) {
  if (tag === 1)
    obj.svgProps = SeatMap.SVGProps.read(pbf, pbf.readVarint() + pbf.pos)
  else if (tag === 2)
    obj.seatProps = SeatMap.SeatProps.read(pbf, pbf.readVarint() + pbf.pos)
  else if (tag === 3)
    obj.groups.push(SeatMap.Group.read(pbf, pbf.readVarint() + pbf.pos))
}
SeatMap.write = function (obj, pbf) {
  ...
  if (obj.groups)
    for (var i = 0; i < obj.groups.length; i++)
      pbf.writeMessage(3, SeatMap.Group.write, obj.groups[i])
}
...
SeatMap.SeatProps.read = function (pbf, end) {
  return pbf.readFields(
    SeatMap.SeatProps._readField,
    { r: 0, fill: '', stroke: '', strokeWidth: 0 },
    end
  )
}
